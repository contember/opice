...
### CLI (`packages/cli/`)
The `opice` binary: `init` (scaffold `opice.config.json` + optional GH workflow, now with a tiered run matrix), `test` (wraps `bun test`, injects `OPICE_*` from config + git incl. `OPICE_TIER` from `--tier` and `OPICE_SELECT` from `--select`, finalizes the run), `failures` (pull a failed run for re-eval), `users create` (mint a dashboard login via the admin token), `install-skills` (fetch the skills below into a target repo's `.claude/`).
